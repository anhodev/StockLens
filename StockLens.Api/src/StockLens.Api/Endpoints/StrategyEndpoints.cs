...
public static class StrategyEndpoints
{
    ...
    private static async Task<IResult> CreateStrategy(
        UpsertStrategyRequest req, IApplicationDbContext db, ILoggerFactory loggerFactory, CancellationToken ct)
    {
        ...
        return Results.Created($"/api/strategies/{s.Id}", s.ToDto());
    }

    private static async Task<IResult> UpdateStrategy(
        Guid id, UpsertStrategyRequest req, IApplicationDbContext db, ILoggerFactory loggerFactory, CancellationToken ct)
    {
    ...
    private static async Task<IResult> DeleteStrategy(
        Guid id, IApplicationDbContext db, ILoggerFactory loggerFactory, CancellationToken ct)
    {
        var s = await db.Strategies.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (s is null) return Results.NotFound();
